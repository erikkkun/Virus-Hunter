...
pygame.display.set_caption("  DISEASE HUNTER   HP: " + str(PLAYER_HP) + "  POINTS: 0  "  + messageList[MESSAGE_NUMBER])
...
for _ in range(5):
  randFlu = Influenza(20, random.randint(0, WIDTH), random.randint(0, HEIGHT))
  virusGroup.add(randFlu)

randEbola = Ebola(40, random.randint(0, WIDTH), random.randint(0, HEIGHT))
virusGroup.add(randEbola)


for _ in range(2): #pills
    randPill = pillGoodie("pill.png",random.randint(0, WIDTH), random.randint(0, HEIGHT))
...
while running:
  ...
  if (length == 0):
    ...
    if (MAP_NUMBER <= 3):
      loading_background = "loadingScreen" + str(MAP_NUMBER-1) + ".png"
      ...
      if (v.health <= 0):
        ...
        if v.ogHealth // 2 > PARTICLE_DMG: # not one-shottable
          if isinstance(v, Ebola):
            virusGroup.add(Ebola(v.ogHealth // 2, v.rect.x, v.rect.y))
            virusGroup.add(Ebola(v.ogHealth // 2, v.rect.x, v.rect.y))
            virusGroup.add(Ebola(v.ogHealth // 2, v.rect.x, v.rect.y))
            virusGroup.add(Ebola(v.ogHealth // 2, v.rect.x, v.rect.y))
          ...
          if isinstance(v, Influenza):
            virusGroup.add(Influenza(v.ogHealth // 2, v.rect.x, v.rect.y))
            virusGroup.add(Influenza(v.ogHealth // 2, v.rect.x, v.rect.y))
